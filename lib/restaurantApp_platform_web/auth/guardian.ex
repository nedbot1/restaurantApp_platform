defmodule RestaurantAppPlatformWeb.Auth.Guardian do
 use Guardian, otp_app: :restaurantApp_platform
  ...
  case Accounts.get_account_by_email(email)do
    ...
    account ->
      case validate_password(password, account.hash_password)do
        true -> create_token(account, :accessToken)
 ...
 defp token_options(type)do
  case type do
    :accessToken -> [token_type: "accessToken", ttl: {2, :hour}]
    :resetToken -> [token_type: "resetToken", ttl: {15, :minute}]
    :adminToken -> [token_type: "adminToken", ttl: {30, :day}]
  end
 end
